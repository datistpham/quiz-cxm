    ...
    Mutation: {
        ...
        createClass: async (parent, args, context)=> {
            ...
            try {
                ...
                const [rows]= await connection.execute("INSERT INTO `classes` VALUES(?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE id_class= VALUES(id_class), class_name= VALUES(class_name), description= VALUES(description), perform= VALUES(perform), invite= VALUES(invite), own_id= VALUES(own_id), code_invite= VALUES(code_invite), cover_image= VALUES(cover_image)", [id_class, args?.class_name, args?.description, args?.perform, args?.invite, args?.own_id, code_invite, args.cover_image])
